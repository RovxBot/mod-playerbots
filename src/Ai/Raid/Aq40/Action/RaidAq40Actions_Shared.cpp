...
bool Aq40TrashAvoidDangerousAoeAction::Execute(Event /*event*/)
{
    ...
    for (ObjectGuid const guid : encounterUnits)
    {
        ...
        {
            float const targetDistance = 20.0f;
            float const gap = targetDistance - bot->GetDistance2d(unit);
        ...
        {
            float const targetDistance = 24.0f;
            float const gap = targetDistance - bot->GetDistance2d(unit);
            if (gap > highestThreatGap)
            {
                highestThreatGap = gap;
    ...
    bot->AttackStop();
    bot->InterruptNonMeleeSpells(true);
    return MoveAway(danger, highestThreatGap + 2.0f);
}
