...
float Aq40TwinEmperorsMultiplier::GetValue(Action* action)
{
    ...
    bool const localTwinCombat =
        twinCombatActive || bot->IsInCombat() || Aq40BossHelper::IsEncounterCombatActive(bot);
    if (!localTwinCombat &&
        !Aq40BossHelper::HasAnyNamedUnit(botAI, activeUnits, { "emperor vek'nilash", "emperor vek'lor" }))
        return 1.0f;
